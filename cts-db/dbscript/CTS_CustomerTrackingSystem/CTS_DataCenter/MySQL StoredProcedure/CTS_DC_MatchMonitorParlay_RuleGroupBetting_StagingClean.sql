...
DROP PROCEDURE IF EXISTS `CTS_DC_MatchMonitorParlay_RuleGroupBetting_StagingClean`;
DELIMITER $$

CREATE DEFINER=`ctsOwner`@`%` PROCEDURE `CTS_DC_MatchMonitorParlay_RuleGroupBetting_StagingClean`(
...
BEGIN
	/*
		...
		Task :		Match Monitor Rule - Clean Stagging Data
		DB:			CTS_DataCenter
		Original:
		...
		Example:
			CALL CTS_DC_MatchMonitorParlay_RuleGroupBetting_StagingClean(@ip_LiveIndicator:=0,@ip_MaxSequenceID:=4);
            
	*/
    DECLARE CONS_LOG 									TINYINT DEFAULT 0; #0:LogOff, 1:LogOn
    DECLARE CONST_RULE_GROUPID  						TINYINT DEFAULT 7; # GroupBetting
    ...
    #==================LOG=======================================================
    DECLARE lv_SPName VARCHAR(100) DEFAULT 'CTS_DC_MatchMonitorParlay_RuleGroupBetting_StagingClean';
    IF CONS_LOG = 1 THEN     
    ...
    WHERE st.RuleGroupID = CONST_RULE_GROUPID
		AND st.RuleStatus = 1
    LIMIT 1;
    ...
    IF (ip_LiveIndicator = 1) THEN 		
		...
		FROM 	CTS_DataCenter.MatchMonitorParlayStagingGroupBettingLive mms
		WHERE	mms.SequenceID <= ip_MaxSequenceID
			AND mms.GroupID <> 0
		...
		IF lv_LastTransID > 0 THEN	
			...
			WHERE 	sys.ParameterID = CONST_LASTSEQUENCE_SYSPARAMETERID_LIVE;
		END IF;        
	END IF;
    ...
    IF (ip_LiveIndicator = 0) THEN     
		...
		INSERT INTO Temp_LastGroupIDGroup(MatchID, ScoreDiff, BettypeID, BetID, Betteam, GroupID)
		SELECT 	DISTINCT mms.MatchID,  mms.ScoreDiff, mms.BettypeID, mms.BetID, mms.Betteam, mms.GroupID
		FROM 	CTS_DataCenter.MatchMonitorParlayStagingGroupBettingNonLive mms
		WHERE	mms.SequenceID <= ip_MaxSequenceID
			AND mms.GroupID <> 0
			AND	mms.TransDateToSecond BETWEEN lv_FromTime AND lv_ToTime; 
		...
		IF lv_LastTransID > 0 THEN
			...
			UPDATE 	CTS_DataCenter.SystemParameter AS sys
			SET 	sys.ParameterValue = ip_MaxSequenceID
			WHERE 	sys.ParameterID = CONST_LASTSEQUENCE_SYSPARAMETERID_NONLIVE;
