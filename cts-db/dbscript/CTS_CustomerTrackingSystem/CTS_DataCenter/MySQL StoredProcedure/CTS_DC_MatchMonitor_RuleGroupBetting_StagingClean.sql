...
DROP PROCEDURE IF EXISTS `CTS_DC_MatchMonitor_RuleGroupBetting_StagingClean`;
DELIMITER $$

CREATE DEFINER=`ctsOwner`@`%` PROCEDURE `CTS_DC_MatchMonitor_RuleGroupBetting_StagingClean`(
...
BEGIN
	/*
		...
		Example:
			CALL CTS_DC_MatchMonitor_RuleGroupBetting_StagingClean(@ip_LiveIndicator:=1,@ip_MaxSequenceID:=119780351005);
            
	*/
    DECLARE CONS_LOG 			TINYINT DEFAULT 0; #0:LogOff, 1:LogOn
    DECLARE CONST_RULE_GROUPID  TINYINT DEFAULT 1; # GroupBetting
    ...
    #==================LOG=======================================================
    DECLARE lv_SPName VARCHAR(100) DEFAULT 'CTS_DC_MatchMonitor_RuleGroupBetting_StagingClean';
    IF CONS_LOG = 1 THEN     
    ...
    IF (ip_LiveIndicator = 1) THEN 		
		...
		FROM 	CTS_DataCenter.MatchMonitorStagingGroupBettingLive mms
		WHERE	mms.SequenceID <= ip_MaxSequenceID
			AND mms.GroupID <> 0
    ...
    IF (ip_LiveIndicator = 0) THEN     
		...
		INSERT INTO Temp_LastGroupIDGroup(MatchID, ScoreDiff, BettypeID, BetID, Betteam, GroupID)
		SELECT 	DISTINCT mms.MatchID,  mms.ScoreDiff, mms.BettypeID, mms.BetID, mms.Betteam, mms.GroupID
		FROM 	CTS_DataCenter.MatchMonitorStagingGroupBettingNonLive mms
		WHERE	mms.SequenceID <= ip_MaxSequenceID
			AND mms.GroupID <> 0
			AND	mms.TransDateToSecond BETWEEN lv_FromTime AND lv_ToTime; 
