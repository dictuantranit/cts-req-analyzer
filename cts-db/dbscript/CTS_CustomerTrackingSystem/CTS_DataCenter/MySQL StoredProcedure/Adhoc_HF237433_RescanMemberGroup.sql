...
DROP PROCEDURE IF EXISTS `Adhoc_HF237433_RescanMemberGroup`;

DELIMITER $$
CREATE DEFINER=`ctsOwner`@`%` PROCEDURE `Adhoc_HF237433_RescanMemberGroup`(	
) 
    SQL SECURITY INVOKER
sp : BEGIN
	/* 
		Created:	20250903@Thomas.Nguyen
		Task:		Scan Member  [Redmine ID: #237433]
		DB:			CTS_DataCenter
		Original:

		Revisions:
            - 20250903@Thomas.Nguyen: Created [Redmine ID: #237433]

		Param's Explanation (filtered by):

		Example:
			- CALL  Adhoc_HF237433_RescanMemberGroup();
	*/
    ...
    FROM CTS_DataCenter.Adhoc_HF237433_AssociatedGroupAccount_Group AS gp
   ...
   DO
		...
		INSERT INTO CTS_DataCenter.Adhoc_HF237433_AssociatedGroupAccount_Target(GroupID,CTSCustID,IsAuto,Remark,Created,CreatedBy,LastModifiedDate,LastModifiedBy,IsScaned)
		...
		IF lv_HasDevice = 1 THEN
			...
			WHERE EXISTS (	SELECT 1
							FROM CTS_DataCenter.AssociationByDevice AS lv1  
								INNER JOIN CTS_DataCenter.Adhoc_HF237433_AssociatedGroupAccount_Target AS tg ON tg.CTSCustID = lv1.CTSCustID AND tg.GroupID = lv_ScanGroupID
		...
		IF lv_HasAI = 1 THEN
			...
			WHERE EXISTS (SELECT 1
							FROM CTS_DataCenter.CTSCustomer AS cus
								INNER JOIN CTS_DataCenter.Adhoc_HF237433_AssociatedGroupAccount_Target AS tg ON tg.CTSCustID = cus.CTSCustID AND tg.GroupID = lv_ScanGroupID
		...
		UPDATE CTS_DataCenter.Adhoc_HF237433_AssociatedGroupAccount_Group AS up
        SET ProcessStatus = 1
