...
DROP PROCEDURE IF EXISTS `DCS_DC_Transform_UserAgent_Insert`;

DELIMITER $$
CREATE DEFINER=`ctsOwner`@`%` PROCEDURE `DCS_DC_Transform_UserAgent_Insert`(
...
BEGIN
    ...
    */
	...
	FROM DCS_DataCenter.RawTransaction	AS rt
		LEFT JOIN	DCS_DataCenter.UserAgent AS ua ON ua.UserAgentKey = MD5(LOWER(rt.UserAgent))
