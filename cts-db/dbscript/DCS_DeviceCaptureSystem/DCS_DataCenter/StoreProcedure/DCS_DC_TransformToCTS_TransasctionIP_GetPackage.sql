...
DROP PROCEDURE IF EXISTS `DCS_DC_TransformToCTS_TransactionIP_GetPackage`;

DELIMITER $$
CREATE DEFINER=`ctsOwner`@`%` PROCEDURE `DCS_DC_TransformToCTS_TransactionIP_GetPackage`(IN ip_NoOfTransaction int,IN ip_NoOfBatch int)
...
BEGIN
	...
	*/
	...
	DECLARE		vrSPName		VARCHAR(200)  DEFAULT 'DCS_DC_TransformToCTS_TransactionIP_GetPackage' ;
